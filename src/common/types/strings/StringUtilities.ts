declare type char<Value extends string> = nstring<Value, 1>;

declare type charless<
  Value extends string,
  Char extends string,
> = Value extends `${string}${char<Char>}${string}` ? never : Value;

declare type stringful<Value extends string> = Value extends "" ? never : Value;
...
declare type dotless<Value extends string> = charless<Value, ".">;

declare type slashless<Value extends string> = charless<Value, "/">;

declare type nstring<
  Value extends string,
  Count extends number,
> = _stringhelpers.stringlen<Value> extends Count ? Value : never;
