...
export default function (host: Url["host"], path: string) {
  const processors = {
    "amazon.com": path => (([, pid = null]) => pid === null ? path : ["/dp/", pid.split("/")[0]!].join(""))(path.split("/dp/")),
    "dhl.com": path => path.endsWith("/tracking.html") ? "/tracking.html" : path.startsWith("/orders/") && path.includes("/details/") ? ["tracking.html?tracking-id=", path.split("/")[4]!].join("") : path,
