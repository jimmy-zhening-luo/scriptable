...
namespace _Pack_Tool {
  ...
  export function pack(): void {
    ...
    try {
      ...
      createNewDistFolder();
      filterNonJsFilesFromDistFolder(
        moveBuiltFilesToDistFolder()
      );
      console.log(
    ...
    function createNewDistFolder(): void {
      try {
        ...
        throw new Error(
          `Error creating new './dist' folder: ${e}`
        );
      }
    }
    ...
    function moveBuiltFilesToDistFolder(): number {
      try {
        ...
        if (builtFileCounter === 0)
          ...
          return builtFileCounter;
      } catch (e) {
        throw new Error(
          `Error copying all built files to './dist': ${e}`
        );
      }
    }
    ...
    function filterNonJsFilesFromDistFolder(builtFileCounter: number): void {
      try {
        ...
        throw new Error(
          `Error filtering copied files in './dist': ${e}`
        );
      }
    }
  }
